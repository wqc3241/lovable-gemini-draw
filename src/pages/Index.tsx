              ...
              <TabsContent value="generate" className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button variant="outline" size="sm" onClick={handleCopyPromptText} disabled={!prompt.trim()} className="h-7 gap-1.5 text-xs">
                      <Copy className="h-3.5 w-3.5" />
                      Copy Prompt
                    </Button>
                  </div>
                  <Textarea id="prompt" placeholder="Describe the image you want to generate... (You can paste images here with Ctrl+V)" value={prompt} onChange={e => setPrompt(e.target.value)} onPaste={handlePaste} className="min-h-[120px] resize-none bg-background" />

                  {/* Display pasted images */}
                  {pastedImages.length > 0 && <div className="mt-2">
                      <p className="text-xs text-muted-foreground mb-2">
                        Pasted images ({pastedImages.length}):
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {pastedImages.map((img, idx) => <div key={idx} className="relative group">
                            <img src={img} alt={`Pasted ${idx + 1}`} className="h-20 w-20 rounded border border-border object-cover" />
                            <button onClick={() => handleRemovePastedImage(idx)} className="absolute -top-2 -right-2 bg-destructive text-destructive-foreground rounded-full h-5 w-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity">
                              ×
                            </button>
                          </div>)}
                      </div>
                ...
                <Button onClick={handleGenerate} onTouchEnd={handleTouchGenerate} disabled={isGenerating} className="w-full bg-gradient-to-r from-primary to-accent-foreground text-primary-foreground hover:opacity-90 active:scale-95 transition-transform touch-manipulation" style={{
              ...
              <TabsContent value="edit" className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <label htmlFor="image-upload" className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-background p-8 transition-colors hover:border-primary hover:bg-accent">
                      {uploadedImage ? <img src={uploadedImage} alt="Uploaded" className="max-h-48 rounded-lg" /> : <>
                ...
                <div>
                  ...
                  <Textarea id="edit-prompt" placeholder="Describe how you want to edit the image... (You can paste reference images here with Ctrl+V)" value={prompt} onChange={e => setPrompt(e.target.value)} onPaste={handlePaste} className="min-h-[120px] resize-none bg-background" />
                  ...
                  {pastedImages.length > 0 && <div className="mt-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {pastedImages.map((img, idx) => <div key={idx} className="relative group">
                            <img src={img} alt={`Reference ${idx + 1}`} className="h-20 w-20 rounded border border-border object-cover" />
                            <button onClick={() => handleRemovePastedImage(idx)} className="absolute -top-2 -right-2 bg-destructive text-destructive-foreground rounded-full h-5 w-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity">
                              ×
                            </button>
                          </div>)}
                ...
                <Button onClick={handleGenerate} onTouchEnd={handleTouchGenerate} disabled={isGenerating || !uploadedImage} className="w-full bg-gradient-to-r from-primary to-accent-foreground text-primary-foreground hover:opacity-90 active:scale-95 transition-transform touch-manipulation" style={{
                WebkitTapHighlightColor: 'transparent'
              }} size="lg">
                  {isGenerating ? <>
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ...
              <TabsContent value="prompt" className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    reader.readAsDataURL(file);
                  }} className="hidden" id="prompt-image-upload" />
                    <label htmlFor="prompt-image-upload" className="flex cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-border bg-background p-8 transition-colors hover:border-primary hover:bg-accent">
                  ...
                  {promptImage && <div className="mt-4">
                      <img src={promptImage} alt="Image to analyze" className="w-full rounded-lg border border-border" />
                    </div>}
                </div>

                <Button onClick={handleAnalyzeImage} disabled={isAnalyzing || !promptImage} className="w-full bg-gradient-to-r from-primary to-accent-foreground text-primary-foreground hover:opacity-90" size="lg">
                  {isAnalyzing ? <>
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                      Analyzing Image...
                    </> : <>
                ...
                {analyzedPrompt && <div className="mt-6 space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button onClick={handleUsePromptInGenerate} className="flex-1 bg-gradient-to-r from-primary to-accent-foreground">
                        <Sparkles className="mr-2 h-4 w-4" />
                        Use in Generate
                      </Button>
                    </div>
          ...
          <Card ref={resultSectionRef} className="border-border bg-card p-6 shadow-lg">
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                {generatedImages.length > 0 && <Button onClick={handleDownloadAll} onTouchEnd={handleTouchDownload} disabled={isDownloading} size="default" className="gap-2">
                    {isDownloading ? <>
            ...
            minHeight: "400px"
              ...
              {isGenerating ? <div className="flex h-full min-h-[400px] items-center justify-center">
              <div className="text-center">
                ...
                </div> : <div className="flex h-full min-h-[400px] items-center justify-center text-muted-foreground">
                  <div className="text-center">
        ...
        <DialogContent className="max-w-[95vw] max-h-[95vh] p-0 border-0">
          <div className="relative w-full h-full flex items-center justify-center">
            {generatedImages[currentImageIndex] && <>
                <img src={generatedImages[currentImageIndex]} alt="Generated fullscreen" className="max-w-full max-h-[95vh] object-contain" onClick={() => setIsFullscreenOpen(false)} />
